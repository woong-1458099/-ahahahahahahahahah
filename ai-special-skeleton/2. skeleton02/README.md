# 지식 증류(Distillation)와 LoRA 파인튜닝

Teacher Model(거대 모델)의 윤리적 판단 기준과 지식 등을 Student Model(소형 모델)에 이식(Distillation)하는 방법론인 지식 증류(Knowledge Distillation) 방법론들 중에서 학습 데이터를 Teacher Model로 생성하여 Student Model의 학습에 사용하는 Pseudo Labeling 방법론을 이용하여 학습을 진행합니다. 모델 학습은 LoRA(Low-Rank Adaptation) 방법론을 이용하여 최대한 모델의 본래 능력을 상실하지 않고 현재 풀고자 하는 task에 최적화된 모델로 만드는 방법론으로 진행합니다.

## 프로젝트 목표

1. 실습명 : Text-to-SQL task에 대해서 knowledge distillation 데이터셋 통한 LoRA 학습하기
2. 핵심 주제:
    1. 데이터셋 불러오기 및 EDA
...
├── PEFT_Text2SQL.ipynb             # 학습 관련된 코드드
...
```bash
uv add numpy==2.1.0 pandas==2.2.3 peft==0.17.1 torch==2.9.0 transformers==4.56.0 accelerate==1.10.1 datasets==4.0.0 trl==0.22.2 sqlparse==0.5.5 nltk==3.9.2 jupyter==1.1.1 ipykernel==7.1.0
```
...
모델 훈련은 `PEFT_Text2SQL.ipynb` 파일을 이용하여 진행합니다. 각 셀마다 설명이 포함되어 있기 때문에 해당 부분을 읽어가면서 진행해주시길 바랍니다.
