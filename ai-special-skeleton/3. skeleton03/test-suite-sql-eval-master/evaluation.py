...
def print_scores(scores, etype, include_turn_acc=True):
    ...
    levels = ["easy", "medium", "all", "joint_all"]
    if include_turn_acc:
...
def save_evaluation_results(entries, scores, etype, gold_file, pred_file):
    """평가 결과를 JSON 파일로 저장"""

    # 정답만 필터링 (평가 타입에 따라 다른 필드 확인)
    if etype == "exec":
        correct_entries = [entry for entry in entries if entry.get("exec") == 1]
    elif etype == "match":
        correct_entries = [entry for entry in entries if entry.get("exact") == 1]
    else:  # etype == "all"
        correct_entries = [
            entry
            for entry in entries
            if entry.get("exact") == 1 or entry.get("exec") == 1
        ]
    ...
    results = {
        ...
        "summary": {"scores": {}},
        "correct_predictions": correct_entries,
        "all_predictions": entries,
    }

    # 점수 정보 추가
    for level in ["easy", "medium", "all", "joint_all"]:
        if level in scores:
            results["summary"]["scores"][level] = {
                "count": scores[level]["count"],
                "exec": scores[level].get("exec", 0),
                "exact": scores[level].get("exact", 0),
            }
    ...
    # print(f"\n평가 결과가 {output_filename}에 저장되었습니다.")
    # if len(entries) > 0:
    #     accuracy = len(correct_entries) / len(entries) * 100
    #     print(
    #         f"전체 {len(entries)}개 중 {len(correct_entries)}개 정답 (정확도: {accuracy:.2f}%)"
    #     )
...
def evaluate(
    ...
    include_turn_acc = False  # len(glist) > 1
    ...
    # JSON 파일로 평가 결과 저장
    save_evaluation_results(entries, scores, etype, gold, predict)
